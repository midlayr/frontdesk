/* Midlayr Chat · drop-in widget · served from cdn.midlayr.com/chat.js
  ...
  function boot(cfg) {
    ...
    sh.innerHTML = `
      <style>
        ...
        .w{position:fixed;right:28px;bottom:28px;width:min(380px,calc(100vw - 56px));height:min(560px,calc(100vh - 56px));display:none;flex-direction:column;background:${paper};color:${ink};border:1px solid #E3E1DC;box-shadow:0 12px 32px rgba(0,0,0,.16);font:14px/1.5 system-ui,sans-serif;z-index:2147483001}
        .w.open{display:flex}
        .h{display:flex;align-items:center;gap:10px;padding:12px 14px;background:${ink};color:${paper};font:700 13px system-ui;letter-spacing:.06em;text-transform:uppercase}
        ...
        .t small{display:block;font:10px ui-monospace,monospace;letter-spacing:.06em;color:#9AA0A8;margin-bottom:4px}
        .t.you{align-self:flex-end;background:${color};color:#fff;border-color:${color}}.t.you small{color:rgba(255,255,255,.7)}
        ...
        .c button{border:1px solid ${color};background:#fff;color:${color};border-radius:2px;padding:6px 11px;font:13px system-ui;cursor:pointer}
    ...
    function connect() {
      ...
      ws.onmessage = e => { const m = JSON.parse(e.data);
        ...
        if (m.type === 'joined') { const d = document.createElement('div'); d.className = 'sys'; d.textContent = `— ${m.repName} joined the conversation —`; M.appendChild(d); setLive(m.repName); }
        if (m.type === 'ended') { I.placeholder = 'This chat has ended'; I.disabled = true; }
        M.scrollTop = M.scrollHeight; };
      ws.onclose = () => { ws = null; setTimeout(connect, 2000); };
    }
    function turn(t) { const d = document.createElement('div'); d.className = 't ' + (t.who === 'visitor' ? 'you' : t.who); d.innerHTML = `<small>${t.who === 'visitor' ? 'YOU' : t.who === 'rep' ? esc((cfg.orgName || '').toUpperCase()) + ' · REP' : esc((cfg.orgName || '').toUpperCase())}</small>${esc(t.text)}`; M.appendChild(d); }
