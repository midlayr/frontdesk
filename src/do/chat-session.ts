...
type SessionState = { orgId: string; flowId: string; flowVersion: number; steps: Step[]; stepIdx: number; captured: Record<string, string>; turns: Turn[]; state: 'bot' | 'live' | 'done'; leadId?: string; repId?: string; visitor: Record<string, unknown> };
...
export class ChatSession extends DurableObject<Env> {
  ...
  async fetch(req: Request) {
    ...
    if (role === 'visitor' && this.s.turns.length === 0) await this.botAsk();
    return new Response(null, { status: 101, webSocket: client });
  }

  async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer) {
    const role = this.sockets.get(ws) ?? (ws as any).deserializeAttachment?.() ?? 'visitor';
  ...
  private async visitorSays(text: string) {
    ...
    if (this.s.state !== 'bot') return this.save();
  ...
  private async repSays(text: string, repId: string) { this.s.repId = repId; this.push({ who: 'rep', text, at: Date.now() }); await this.save(); }
  ...
  private async load(url: URL) {
    ...
    const orgId = url.searchParams.get('org')!; const flowSlug = url.searchParams.get('flow') || 'quote-intake';
    ...
    this.s = { orgId, flowId: cfg.id, flowVersion: cfg.version, steps: cfg.steps, stepIdx: 0, captured: {}, turns: [], state: 'bot', visitor: { referrer: url.searchParams.get('ref'), landing: url.searchParams.get('page'), ua: url.searchParams.get('ua') } };
    await this.save();
  }
  ...
  private async ensureLead() {
    ...
    const r = await this.env.INTERNAL.fetch('https://internal/leads/from-chat', { method: 'POST', body: JSON.stringify({ orgId: this.s.orgId, sessionId: this.ctx.id.toString(), captured: this.s.captured, visitor: this.s.visitor, flowId: this.s.flowId, flowVersion: this.s.flowVersion }) });
    this.s.leadId = (await r.json() as any).leadId;
    await this.env.JOBS.send({ kind: 'extract_specs', orgId: this.s.orgId, leadId: this.s.leadId! });
  }
  private async persist() { if (!this.s?.leadId) return; await this.env.INTERNAL.fetch('https://internal/leads/chat-turns', { method: 'POST', body: JSON.stringify({ leadId: this.s.leadId, turns: this.s.turns, state: this.s.state, captured: this.s.captured }) }); }
  private async notifyInbox(kind: string) { const room = this.env.INBOX_ROOM.get(this.env.INBOX_ROOM.idFromName(this.s.orgId)); await room.fetch('https://internal/push', { method: 'POST', body: JSON.stringify({ kind, leadId: this.s.leadId, sessionId: this.ctx.id.toString(), captured: this.s.captured, state: this.s.state }) }); }
}
